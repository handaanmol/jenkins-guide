...
// Performing Software Tests
    stage('TEST') {
      parallel {
        stage('Mocha Tests') {
          steps {
            sh 'sudo docker run --name nodeapp-dev --network="bridge" -d \
            -p 9000:9000 nodeapp-dev:trunk'
            sh 'sudo docker run --name test-image -v $PWD:/JUnit --network="bridge" \
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'sudo docker tag nodeapp-dev:trunk anmolhanda/nodeapp-dev:latest'
            sh 'sudo docker push anmolhanda/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'sudo docker tag nodeapp-dev:trunk anmolhanda/nodeapp-prod:latest'
                            sh 'sudo docker push anmolhanda/nodeapp-prod:latest'
                            sh 'sudo docker save anmolhanda/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
